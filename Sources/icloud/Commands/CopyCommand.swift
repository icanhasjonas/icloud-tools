...
struct CopyCommand: ParsableCommand {
    ...
    func run() throws {
        ...
        if sources.count > 1 && (!destExists || !destIsDir.boolValue) {
            throw ValidationError("Destination must be a directory when copying multiple files.")
        }
        ...
        for source in sources {
            ...
            if srcIsDir.boolValue && !recursive {
                throw ValidationError("\(srcURL.lastPathComponent) is a directory (use -r to copy recursively)")
            }
            ...
            try fm.copyItem(at: srcURL, to: finalDest)

            if verbose {
                print("\(Output.green)copied\(Output.reset) \(srcURL.lastPathComponent) -> \(finalDest.path)")
            }
        }
    }
}
